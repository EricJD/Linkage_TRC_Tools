...
public class Cooccs {
	...
	// this map contains a mapping term->Map::String->Double, i.e. each term has a
	// number of co-occurring items
	// associated with it each of which is in turn associated with a co-occurrence
	// frequency.
	private Map cooccs;
	...
	/**
	 * Constructor that will calculate co-occurrences from a file containing
	 * sentences.
	 * 
	 * @param satzFile The file that contains the initial corpus, one sentence per
	 *                 line
	 */
	public Cooccs(File satzFile, boolean usedb) {
		...
		try {
			...
			for (int i = 0; i < phrases.size(); i++) {
				...
				if (phrases.get(i).toString().endsWith("A N"))
					lower = true;
			...
			while ((lineorig = myInput2.readLine()) != null) {
				...
				for (int i = 0; i < phrases.size(); i++) {
					while (strbuf.toString()
							/* .toLowerCase() */.indexOf(((String) phrases.get(i))/* .toLowerCase() */) != -1) {

						int k = strbuf.toString()
								/* .toLowerCase() */.indexOf(((String) phrases.get(i))/* .toLowerCase() */);
			...
			Vector<String> cleanwordlist = new Vector<String>();
				...
				if (!lineorig.equals("")) {
					...
					for (int i = 0; i < splittedsentence.length; i++) {
						...
						if ((splittedsentence[i].indexOf("##phrase") != -1)) {
							...
							if ((splittedsentence[i].indexOf("|NN") != -1) || (splittedsentence[i].indexOf("|NE") != -1)
									|| (splittedsentence[i].indexOf("|NP") != -1)) {
										...
										if (!splittedsentence[i].substring(0, pos).toLowerCase().equals("%n%")) {
											...
											if (splittedsentence[i].indexOf("|NP") != -1) {
												...
												if (!isStopWord(splittedsentence[i].substring(0, pos))) {
													...
													allwords.add(splittedsentence[i].substring(0, pos)
															+ "|NP"/* .toLowerCase() */);
												} else {
							...
							/*
							 * if ((splittedsentence[i].indexOf("|ADJ")!=-1) ||
							 * (splittedsentence[i].indexOf("|AJ")!=-1)) {
							 * 
							 * int pos = splittedsentence[i].indexOf("|");
							 * 
							 * if (pos>0) if (splittedsentence[i].substring(0, pos).length()>1) if
							 * (!splittedsentence[i].substring(0, pos).toLowerCase().equals("%n%")) {
							 * 
							 * if (!isStopWord(splittedsentence[i].substring(0, pos))) {
							 * 
							 * 
							 * allwords.add(splittedsentence[i].substring(0, pos).toLowerCase()+"|ADJ"); }
							 * else { // System.out.println("Stopword removed: "+splittedsentence[i]); } } }
							 *//* else */// find adverbs
							/*
							 * if ((splittedsentence[i].indexOf("|ADV")!=-1) ||
							 * (splittedsentence[i].indexOf("|AV")!=-1)) {
							 * 
							 * int pos = splittedsentence[i].indexOf("|");
							 * 
							 * if (pos>0) if (splittedsentence[i].substring(0, pos).length()>1) if
							...
							/* else */ // find verbs
							/*
							 * if ( (splittedsentence[i].indexOf("|VV")!=-1) ||
							 * (splittedsentence[i].indexOf("|VA")!=-1) ||
							 * (splittedsentence[i].indexOf("|VM")!=-1) ){
							 * 
							 * int pos = splittedsentence[i].indexOf("|");
							 * 
							 * if (pos>0) if (splittedsentence[i].substring(0, pos).length()>1) if
					...
					if (cleanwordlist.size() > 1) { 
						...
						//extractCooccs(cleanwordlist); // extract sentence co-occurrences (in-memory)

						LinkedHashSet<String> hashSet
			            = new LinkedHashSet<String>(cleanwordlist);
						
						cleanwordlist.clear();
						cleanwordlist.addAll(hashSet);
						
						System.out.println(cleanwordlist);
						
						if (usedb == true) {
							addSentenceToCooccsDB(cleanwordlist); // extract sentence co-occurrences (Neo4j DB)
						} else
							extractCooccs(cleanwordlist); // extract sentence co-occurrences (in-memory) 
		...
		// if (!usedb)	 //graph database shall not be created
		// calcSigsLL(); //undirected graph 

		// if (!usedb)      //graph database shall not be created
		// calcSigsDICE();	//undirected graph

		if (!usedb)				//graph database shall not be created
			calcAssociations(); //directed graph

		if (usedb == true)		//graph database shall be created
			updateDiceAndCosts();
	...
	/**
	* Extracts the co-occurrences of a sentence and stores them in the 
	* Neo4j default database "neo4j" in the program folder "cooccsdatabase".
	* 
	* @param  words  Vector containing the co-occurring words to be stored.
	*
	*/
	public void addSentenceToCooccsDB(Vector<String> words) // adding words from a sentence to the graph DB
	{
		...
		try (Transaction tx = graphDB.beginTx()) {
			...
			for (Iterator<String> i = words.iterator(); i.hasNext();) {
				...
				if (f_flag == false) {
					...
					wordnode.setProperty("occur", 1); // number of occurrences in database
	...
	*/
	...
	public Vector getAllNodes() {
		...
		// GraphDatabaseFactory().newEmbeddedDatabase(database);
		managementService = new DatabaseManagementServiceBuilder(database).build();
		graphDB = managementService.database(DEFAULT_DATABASE_NAME);
		...
		try (Transaction tx2 = graphDB.beginTx()) {
			ResourceIterator<Node> terms = tx2.findNodes(Labels.SINGLE_NODE);
			while (terms.hasNext()) {
			...
			if (query.size() > 1) {
				...
				try (Transaction tx2 = graphDB.beginTx()) {
					...
					for (int i = 0; i < query.size(); i++) {
						Node temp = tx2.findNode(Labels.SINGLE_NODE, "name", query.get(i).toString());
					...
					if (helpquery.size() == query.size()) {
						...
						while (iteratorq1.hasNext()) {
							...
							Node temp = tx2.findNode(Labels.SINGLE_NODE, "name", queryterm);
							...
							while (iteratorq2.hasNext()) {
								...
								if (!queryterm.equals(queryterm2)) {
									Node temp2 = tx2.findNode(Labels.SINGLE_NODE, "name", queryterm2);

									// PathFinder<Path> finder =
									// GraphAlgoFactory.shortestPath(PathExpanders.forTypeAndDirection(
									// RelationshipTypes.IS_CONNECTED, Direction.BOTH ), 100, 1 );
									PathFinder<Path> finder = GraphAlgoFactory.shortestPath(
											new BasicEvaluationContext(tx2, graphDB), PathExpanders.forTypeAndDirection(
													RelationshipTypes.IS_CONNECTED, Direction.BOTH),
											100, 1);
							...
							if (helpqueryset.size() == query.size()) {
								...
								for (int i = 0; i < query.size(); i++) {
									...
									for (int j = i + 1; j < query.size(); j++) {
										...
										Node temp = tx2.findNode(Labels.SINGLE_NODE, "name", query.get(i).toString());
										Node temp2 = tx2.findNode(Labels.SINGLE_NODE, "name", query.get(j).toString());

										PathExpander<Double> expander = PathExpanders
												.forTypeAndDirection(RelationshipTypes.IS_CONNECTED, Direction.BOTH);

										// PathFinder<WeightedPath> finder = GraphAlgoFactory.dijkstra(
										// PathExpanders.forTypeAndDirection( RelationshipTypes.IS_CONNECTED,
										// Direction.BOTH ), "cost", 1 );
										PathFinder<WeightedPath> finder = GraphAlgoFactory.dijkstra(expander, "cost",
												1);
										// PathFinder<WeightedPath> finder =
										// GraphAlgoFactory.dijkstra((Double)PathExpanders.forTypeAndDirection(
										// RelationshipTypes.IS_CONNECTED, Direction.BOTH ), "cost", 1);
								...
								System.out.println("largestdistanceofqueryterms: " + largestdistanceofqueryterms + "  "
										+ largestpathlength);

								if ((largestdistanceofqueryterms >= minpathlength)
										&& (largestdistanceofqueryterms < maxpathlength)) {
				...
				try (Transaction tx2 = graphDB.beginTx()) {
					...
					if ((query.size() == 1)) {
						...
						for (Iterator i = query.iterator(); i.hasNext();) {
							...
							Node firstsourcenode = tx2.findNode(Labels.SINGLE_NODE, "name", curQueryTerm);
							...
							while ((queue.size() != 0)) {
								...
								System.out.println(
										"polling " + sourcenodename + " step: " + node2step.get(sourcenodename));
								...
								if (step < maxactivationsteps) {
									...
									Node temp = tx2.findNode(Labels.SINGLE_NODE, "name", sourcenodename);
									...
									for (Relationship relationship : allRelationships) {
										...
										if (!visited.contains(destinationnodename)) {
											...
											/*
											 * visited.add(destinationnodename); queue.add(destinationnodename);
											 */
					...
					if (checkNodeExists(distilledText.get(i).toString())) {
						...
						if (existingwords == 1) { // first real word read is first centroid
							...
							if ((i) < distilledText.size()) {
								...
								System.out.println("Calculate shortest path: " + currentEvolvingCentroid.toString()
										+ " " + nextWord.toString());
								;
								...
								if (currentdistance < Double.MAX_VALUE) {
									...
									double delta = (currentdistance / (Math.sqrt(existingwords))); // existing
																									// relations?
									...
									if (currentEvolvingCentroid.get(0).toString().equals(currentEvolvingCentroid.get(1))
											&& (((double) currentEvolvingCentroid.get(2)) == 0.0)) {

										System.out.println("current centroid is on a specific node (no offset)");
										;

										if (nextWord.get(0).toString()
												.equals(currentEvolvingCentroid.get(0).toString())) {
											...
											if (currentPath.get(1).toString().equals(nextWord.get(0).toString())) {
												...
												// intermediate nodes are between the current centroid term and the new
												// word read

												System.out
														.println("word read is NOT current centroid term's neighbour");
												;
												...
												for (k = 1; k < currentPath.size(); k++) {
													...
													System.out.println("reading next word from current path "
															+ currentPath.get(k).toString());
													;
													...
													if (limitkfound == false) {
														...
														double helpdist = getPositionDistance2(currentEvolvingCentroid,
																helpvec);
														System.out.println(
																"distance from current centroid to next word from current path: "
																		+ helpdist);
														;
														...
														if (helpdist >= delta) {
															...
															if (k > 0) {
																...
																double helpdist2 = getPositionDistance2(
																		currentEvolvingCentroid, helpvec2);

																double newoffset = delta - helpdist2;

																Vector nextEvolvingCentroid = new Vector();
																nextEvolvingCentroid
																		.add(currentPath.get(k - 1).toString());
										...
										// check distance from current centroid position to next node on shortest path
										// to next read word
											...
											if (limitkfound == false) {
												...
												if (helpdist >= delta) {
													...
													if (k == 0) {
														...
														;
														// case a)
														if (currentPath.get(k).toString()
																.equals(currentEvolvingCentroid.get(0).toString())) {
															System.out.println("case a)");
															;

															double newoffset = ((double) currentEvolvingCentroid.get(2))
																	- delta;

															Vector nextEvolvingCentroid = new Vector();
															nextEvolvingCentroid
																	.add(currentEvolvingCentroid.get(0).toString());
															nextEvolvingCentroid
																	.add(currentEvolvingCentroid.get(1).toString());
															nextEvolvingCentroid.add(newoffset);
														...
														if (currentPath.get(k).toString()
																.equals(currentEvolvingCentroid.get(1).toString())) {
														...
														double helpdist2 = getPositionDistance2(currentEvolvingCentroid,
																helpvec2);
		...
		try (Transaction tx = graphDB.beginTx()) {
			...
			if (temp != null) {
				...
				if (temp2 != null) {
					...
					PathExpander<Double> expander = PathExpanders.forTypeAndDirection(RelationshipTypes.IS_CONNECTED,
							Direction.BOTH);

					// PathFinder<WeightedPath> finder = GraphAlgoFactory.dijkstra(
					// PathExpanders.forTypeAndDirection( RelationshipTypes.IS_CONNECTED,
					// Direction.BOTH ), "cost", 1 );
					PathFinder<WeightedPath> finder = GraphAlgoFactory.dijkstra(expander, "cost", 1);
					...
					if (p != null) {
						...
						for (Node n : p.nodes()) {
							 ...
							 * System.out.println(n.getProperty("name")+""); } else
	...
	public double getPositionDistance(Vector pos1, Vector pos2) {
		...
		System.out.println(
				"Start:	'" + termstart_pos1 + "' '" + termend_pos1 + "' " + offset_pos1_start + " " + offset_pos1_end);
		...
		System.out.println(
				"Start:	'" + termstart_pos2 + "' " + termend_pos2 + "' " + offset_pos2_start + " " + offset_pos2_end);

		if (termstart_pos1.equals(termend_pos1) && termstart_pos2.equals(termend_pos2) && (offset_pos1_start == 0.0)
				&& (offset_pos2_start == 0.0)) { // shortest node distance
			...
			/*
			 * currentPath needed? currentPath.clear();
			 * 
			 * currentPath.add(termstart_pos1); currentPath.add(termend_pos1);
			 * 
			 */
	...
	// the following 2 methods copy the code from the both methods above, but no
	// currentPath is set
...
//	GraphDatabaseService graphDB = new GraphDatabaseFactory().newEmbeddedDatabase(database);
	...
	public double getPositionDistance2(Vector pos1, Vector pos2) {
		...
		if (termstart_pos1.equals(termend_pos1) && termstart_pos2.equals(termend_pos2) && (offset_pos1_start == 0.0)
			...
			distance = Math
					.abs(getNodeDistance2(termstart_pos1, termstart_pos2) - offset_pos1_start - offset_pos2_start);
			...
			double distancehelp1 = getNodeDistance2(termstart_pos1, termstart_pos2) + offset_pos1_start
					+ offset_pos2_start;
			...
			/*
			 * if (subcase==1) getNodeDistance(termstart_pos1, termstart_pos2);
			 * 
			 * if (subcase==2) getNodeDistance(termstart_pos1, termend_pos2);
			 * 
			 * if (subcase==3) getNodeDistance(termend_pos1, termstart_pos2);
			 * 
			 * if (subcase==4) getNodeDistance(termend_pos1, termend_pos2);
			 * 
			 * System.out.println("CurrentPath	" + currentPath.toString());
			 */
	...
	public HashMap getCentroidbySpreadingActivation(Vector query, boolean allowcentroidcandidatesinquery) {
		...
		if (!query.isEmpty()) {
			...
			if (query.size() > 1) {
				...
				try (Transaction tx2 = graphDB.beginTx()) {
					...
					query = helpquery;
					System.out.println("Number of query terms found in database: " + query.size() + "   Derived query: " + query);
					

					// Check if all query terms can be reached in the graph database from one
					// another
					 ...
					 * 
					 * helpqueryset.remove(term2bremoved); term2bremoved = "";
					 * 
					 * }
					 * 
					 ...
					 * 
					 * while (iterator2.hasNext()){ String queryterm2 = iterator2.next().toString();
					 * 
					 * if (!queryterm.equals(queryterm2)) { Node temp2 =
					 * graphDB.findNode(Labels.SINGLE_NODE,"name", queryterm2);
					 * 
					 * PathFinder<Path> finder =
					 * GraphAlgoFactory.shortestPath(PathExpanders.forTypeAndDirection(
					 * RelationshipTypes.IS_CONNECTED, Direction.BOTH ), 100, 1 );
					...
					query = helpquery2;
					System.out.println("Size of pruned query (reachable nodes in database): " + query.size() + "   Pruned query: " + query);
					...
					if ((query.size() == query.size()) && (query.size() > 1)) {
						...
						System.out.println("Largest distance of query terms (weight): " + largestdistanceofqueryterms + 
								" with path length (hops): " + largestpathlength);
						...
						while ((centroidcandidates.size() < 5) && (count < 10)) {
								...
								while ((queue.size() != 0) && (steps < count)) {
									...
									for (Relationship relationship : allRelationships) {
										...
										if (!visited.contains(destinationnodename)) {
											 ...
											 * );
											 * 
											 * if (p!=null) if ( p.weight() < arearadius) {
											 * 
											 * visited.add(destinationnodename); queue.add(destinationnodename);
											 * 
											 * }
											 ...
											 * }
											 * 
											 * if (contains==query.size()) { queue = new LinkedList<String>(); }
											 */
							...
							while (iterator.hasNext()) {
								...
								// (!helpqueryset.contains(nodename)) means: query terms must not appear in
								// result set of centroid candidates
								if (allowcentroidcandidatesinquery == true)
									if ((((HashSet) node2colors.get(nodename)).size() == termcolors
											.size()) /* && (!helpqueryset.contains(nodename)) */) {
										centroidcandidates.add(nodename);
									}

								if (allowcentroidcandidatesinquery == false)
									if ((((HashSet) node2colors.get(nodename)).size() == termcolors.size())
											&& (!helpqueryset.contains(nodename))) {
										centroidcandidates.add(nodename);
									}
						...
						for (int i = 0; i < centroidcandidates.size(); i++) {
							...
							System.out.println("Starting run for candidate " + candidate);
							averagepathlength = 0;

							Node n1 = tx2.findNode(Labels.SINGLE_NODE, "name", candidate);

							for (int j = 0; j < query.size(); j++) {

								String curQueryTerm = query.get(j).toString();

								Node n2 = tx2.findNode(Labels.SINGLE_NODE, "name", curQueryTerm);
							...
							System.out.println("Path score for " + candidate + ": " + averagepathlength);
		...
		if (!query.isEmpty()) {
			...
			if (query.size() > 1) {
				...
				try (Transaction tx2 = graphDB.beginTx()) {
					...
					int nodenumber = 0;
					ResourceIterator<Node> term1iterator = tx2.findNodes(Labels.SINGLE_NODE);
						...
						if (!query.contains(term1str)) {
							...
							for (Iterator i = query.iterator(); i.hasNext();) {
								...
								if (curnumberofaddedpaths < maximumpathstoadd) {
									...
									Node n1 = tx2.findNode(Labels.SINGLE_NODE, "name", term1str);
									Node n2 = tx2.findNode(Labels.SINGLE_NODE, "name", curQueryTerm);
									...
									if ((n1 != null) && (n2 != null)) {
										...
										// PathFinder<Path> finder = GraphAlgoFactory.shortestPath(
										// Traversal.expanderForTypes( RelationshipTypes.IS_CONNECTED ), 20 );
										 ...
										 * 
										 * for (WeightedPath p: paths) { pathweight=p.weight(); }
										 */
									...
									// System.out.println("Fehler " + curnumberofaddedpaths + " " +
									// maximumpathstoadd);
...
//hierher3
	...
	public void getAllProperties(String n1) {
		...
		System.out.println("database opened for querying");
		try (Transaction tx2 = graphDB.beginTx()) {
			ResourceIterator<Node> nodelist = tx2.findNodes(Labels.SINGLE_NODE);
			while (nodelist.hasNext()) {
				Node wordnode = nodelist.next();
				if (wordnode.getProperty("name").equals(n1)) {
					System.out.println("No. of occurrences of node " + wordnode.getProperty("name") + " are "
							+ wordnode.getProperty("occur"));
					flag = true;
					break;
				}
			...
			if (!flag) {
				...
				Node temp = tx2.findNode(Labels.SINGLE_NODE, "name", n1);
				Iterable<Relationship> allRelationships = temp.getRelationships();
				...
				for (Relationship relationship : allRelationships) {
					...
					System.out.println("Neighbour: " + n2.getProperty("name") + "	\t Co-occurrences: "
							+ relationship.getProperty("count") + " \t Dice: " + dice);
	...
	public HashMap getAllNeighbours(String n1) {
		...
		try (Transaction tx2 = graphDB.beginTx()) {
			...
			while (nodelist.hasNext()) {
				Node wordnode = nodelist.next();
				if (wordnode.getProperty("name").equals(n1)) {
					System.out.println("No. of occurrences of node " + wordnode.getProperty("name") + ": "
							+ wordnode.getProperty("occur"));
					flag = true;
					break;
				}
			...
			if (!flag) {
				...
				for (Relationship relationship : allRelationships) {
					...
					String dice = relationship.getProperty("dice").toString();
					String cost = relationship.getProperty("cost").toString();
					System.out.println(n2.getProperty("name") + "\t" + relationship.getProperty("count") + "\t" + dice
							+ "\t" + cost);
					...
					// System.out.println("Startnode: " +startnode.getProperty("name") + " --->
	...
	public double getShortestPathWeightInDatabase(String node1, String node2) {
		...
		try (Transaction tx2 = graphDB.beginTx()) {
			// rel_found=false;
			Node n1 = tx2.findNode(Labels.SINGLE_NODE, "name", node1);
			Node n2 = tx2.findNode(Labels.SINGLE_NODE, "name", node2);
	...
	public Vector getShortestPathInDatabase(String node1, String node2) {
		...
		managementService.shutdown(); // graphDB
		return path;
	}
	...
	public List getcombinedCooccsList(String query) {
		...
		int endIndex = 20;
		if (retrievedcombined.size() < (endIndex))
			endIndex = retrievedcombined.size();
		sorted.addAll(retrievedcombined.subList(0, endIndex));

		/*
		 * for(Iterator i=sorted.iterator(); i.hasNext(); ){ Word curWord =
		 * (Word)i.next(); String curStr = curWord.getWordStr(); ret.add(curStr);
		 * 
		 * }
		 */
	...
	public List expand(String query, int maxcandidates) {
		...
		if (expansioncandidates.size() < maxcandidates)
			maxcandidates = expansioncandidates.size();
	...
	// calculates the similarity between two terms based on their global contexts
	// via DICE coefficient
	public double sim(String term1, String term2) {
		...
		if (term1cooccs.size() == 0 || term2cooccs.size() == 0)
			return sim;
...
//	 gets co-occurrences for a given term as a List
	public List getCooccsList(String term, int demand) {
		...
		int endIndex = demand;
		if (retrieved.size() < endIndex)
			endIndex = retrieved.size();
	...
	// scans a list of words for stopwords and removes them. Returns the "cleansed"
	// list...
	...
	private void extractCooccs(Vector words) {
		...
		for (Iterator i = words.iterator(); i.hasNext();) {
			...
			// we will try to update this map with all the other words occurring in the
			// current sentence:
	...
	private float sig(float k, int na, int nb) {
		...
		sig = n_log_n(k) - n_log_n(na) - n_log_n(nb) + n_log_n(n) + n_log_n(n - nb - na + k) + n_log_n(na - k)
				+ n_log_n(nb - k) - n_log_n(n - na) - n_log_n(n - nb);
	...
	private void writeLineToFile(String filename, String linetoWrite, boolean appendToFile) {
